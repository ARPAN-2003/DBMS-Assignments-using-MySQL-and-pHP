<?php
    ...
    if (!$conn) {
        ...
        $insert_query = "INSERT INTO Uses (PaperID, LeadAuthorID) VALUES ('$paperId', $empId')";

        if (mysqli_query($conn, $insert_query)) {
            $success = true;
            $message = "Researcher_leads_ResearchPaper Form data is successfully stored in Research Database. Thank You.";
        } else {
            $success = false;
            $message = "Error inserting data: " . mysqli_error($conn);
        }

        $select_query = "SELECT * FROM Leads ORDER BY PaperID";
...
<head>
    ...
    <title>Researcher_leads_ResearchPaper Form Submission Result</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="data-display-section">
            <h3>📊 Inserted Data - Leads Table</h3>

            <table>
                <thead>
                    <tr>
                        <th>Research Paper ID</th>
                        <th>Lead Author ID</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                        if(count($table_data) > 0) {
                            foreach($table_data as $row) {
                                ...
                                echo "<td>" . htmlspecialchars($row['LeadAuthorID']) . "</td>";
                                echo "</tr>";
                            }
                        } else {
        ...
        <div class="button-group">
            ...
            <button class="btn-back" onclick="window.location.href='researcher_leads_researchpaper.html'">↻ &nbsp; Insert another data</button>
        </div>
    </div>
</body>
</html>