<?php
    ...
    if (!$conn) {
        ...
        $equipId = isset($_POST['equipId']) ? mysqli_real_escape_string($conn, $_POST['equipId']) : '';
        $equipName = isset($_POST['equipName']) ? mysqli_real_escape_string($conn, $_POST['equipName']) : '';
        $calibStandard = isset($_POST['calibStandard']) ? mysqli_real_escape_string($conn, $_POST['calibStandard']) : '';

        $insert_query = "INSERT INTO Lab_Equipment (EquipmentID, EquipmentName, CalibrationStandard) VALUES ('$equipId', '$equipName', '$calibStandard')";

        if (mysqli_query($conn, $insert_query)) {
            $success = true;
            $message = "Lab Equipment Form data is successfully stored in Research Database. Thank You.";
        } else {
            $success = false;
            $message = "Error inserting data: " . mysqli_error($conn);
        }

        $select_query = "SELECT * FROM Lab_Equipment ORDER BY EquipmentID";
...
<head>
    ...
    <title>Lab Equipment Form Submission Result</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="data-display-section">
            <h3>📊 Inserted Data - Lab_Equipment Table</h3>
                    ...
                    <?php 
                        if(count($table_data) > 0) {
                            foreach($table_data as $row) {
                                ...
                                echo "<td>" . htmlspecialchars($row['CalibrationStandard']) . "</td>";
                                echo "</tr>";
                            }
                        } else {
        ...
        <div class="button-group">
            ...
            <button class="btn-back" onclick="window.location.href='lab_equipment.html'">↻ &nbsp; Insert another data</button>
        </div>
    </div>
</body>
</html>