<?php
    ...
    if (!$conn) {
        ...
        $insert_query = "INSERT INTO Uses (EmployeeID, EquipmentID) VALUES ('$empId', '$equipId')";

        if (mysqli_query($conn, $insert_query)) {
            $success = true;
            $message = "Researcher_uses_Equipment Form data is successfully stored in Research Database. Thank You.";
        } else {
            $success = false;
            $message = "Error inserting data: " . mysqli_error($conn);
        }

        $select_query = "SELECT * FROM Uses ORDER BY EmployeeID";
...
<head>
    ...
    <title>Researcher_uses_Equipment Form Submission Result</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="data-display-section">
            <h3>📊 Inserted Data - Uses Table</h3>

            <table>
                <thead>
                    <tr>
                        <th>Employee ID</th>
                        <th>Lab Equipment ID</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
        ...
        <div class="button-group">
            ...
            <button class="btn-back" onclick="window.location.href='researcher_uses_equipment.html'">↻ &nbsp; Insert another data</button>
        </div>
    </div>
</body>
</html>