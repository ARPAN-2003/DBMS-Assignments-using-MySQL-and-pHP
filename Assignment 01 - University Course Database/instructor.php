<?php
    ...
    if (!$conn) {
        ...
        $instructorId = isset($_POST['instructorId']) ? mysqli_real_escape_string($conn, $_POST['instructorId']) : '';
        $personId = isset($_POST['personId']) ? mysqli_real_escape_string($conn, $_POST['personId']) : '';

        $insert_query = "INSERT INTO Instructor (InstructorID, PersonID) VALUES ('$instructorId', '$personId')";

        if (mysqli_query($conn, $insert_query)) {
            $success = true;
            $message = "Instructor Form data is successfully stored in Instructor Database. Thank You.";
        } else {
            $success = false;
            $message = "Error inserting data: " . mysqli_error($conn);
        }

        $select_query = "SELECT * FROM Instructor ORDER BY InstructorID";
...
<head>
    ...
    <title>Instructor Form Submission Result</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="data-display-section">
            <h3>📊 Inserted Data - Instructor Table</h3>
        ...
        <div class="button-group">
            ...
            <button class="btn-back" onclick="window.location.href='instructor.html'">↻ &nbsp; Insert another data</button>
        </div>
    </div>
</body>
</html>