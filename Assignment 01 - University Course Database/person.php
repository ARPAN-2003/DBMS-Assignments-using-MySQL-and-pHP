<?php
    ...
    if (!$conn) {
        ...
        $personId = isset($_POST['personId']) ? mysqli_real_escape_string($conn, $_POST['personId']) : '';
        $personName = isset($_POST['personName']) ? mysqli_real_escape_string($conn, $_POST['personName']) : '';
        $dob = isset($_POST['dateOfBirth']) ? mysqli_real_escape_string($conn, $_POST['dateOfBirth']) : '';

        $insert_query = "INSERT INTO Person (PersonID, PersonName, DateOfBirth) VALUES ('$personId', '$personName', '$dob')";

        if (mysqli_query($conn, $insert_query)) {
            $success = true;
            $message = "Person Form data is successfully stored in Person Database. Thank You.";
        } else {
            $success = false;
            $message = "Error inserting data: " . mysqli_error($conn);
        }

        $select_query = "SELECT * FROM Person ORDER BY PersonID";
...
<head>
    ...
    <title>Person Form Submission Result</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="data-display-section">
            <h3>📊 Inserted Data - Person Table</h3>
        ...
        <div class="button-group">
            ...
            <button class="btn-back" onclick="window.location.href='person.html'">↻ &nbsp; Insert another data</button>
        </div>
    </div>
</body>
</html>